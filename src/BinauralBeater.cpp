...
struct BinauralBeater : Module {
  ...
  BinauralBeater() {
    ...
    configParam(BF_KNOB_PARAM, 0.f, 100.f, 4.f, "Beat frequency", " Hz");
    configParam(CF_KNOB_PARAM, 1.f, 20000.f, 261.63f, "Carrier frequency",
                " Hz");
    configParam(AM_RATE_KNOB_PARAM, 0.f, 100.f, 0.f,
                "Amplitude modulation rate", " Hz");
    configParam(AM_DEPTH_KNOB_PARAM, 0.f, 1.f, 0.f,
                "Amplitude modulation depth");
    configParam(FM_RATE_KNOB_PARAM, 0.f, 100.f, 0.f,
                "Frequency modulation rate", " Hz");
    configParam(FM_RANGE_KNOB_PARAM, 0.f, 100.f, 0.f,
                "Frequency modulation range", " Hz");
    configParam(AM_RATE_CV_KNOB_PARAM, 0.f, 1.f, 0.f,
                "Amplitude modulation rate CV amount");
    configParam(AM_DEPTH_CV_KNOB_PARAM, 0.f, 1.f, 0.f,
                "Amplitude modulation depth CV amount");
    configParam(FM_RATE_CV_KNOB_PARAM, 0.f, 1.f, 0.f,
                "Frequency modulation rate CV amount");
    configParam(FM_RANGE_CV_KNOB_PARAM, 0.f, 1.f, 0.f,
                "Frequency modulation range CV amount");
    configSwitch(LR_SWITCH_PARAM, 0.f, 1.f, 0.f, "High frequency output side",
                 {"Left", "Right"});
  ...
  EffectiveControls resolveTargets() {
    ...
    targets.beatHz = applyScaledCv(
        params[BF_KNOB_PARAM].getValue(), inputs[BF_CV_IN_INPUT].getVoltage(),
        params[BF_CV_KNOB_PARAM].getValue(), 0.f, 100.f);

    const float pitchVolts = inputs[V_OCT_IN_INPUT].getVoltage();
    const float baseCarrierHz =
        params[CF_KNOB_PARAM].getValue() * std::pow(2.f, pitchVolts);
    targets.carrierHz = clampFrequency(baseCarrierHz);

    targets.amRateHz = applyScaledCv(params[AM_RATE_KNOB_PARAM].getValue(),
    ...
    targets.fmRateHz = applyScaledCv(params[FM_RATE_KNOB_PARAM].getValue(),
    ...
    targets.fmRangeHz =
        applyScaledCv(params[FM_RANGE_KNOB_PARAM].getValue(),
                      ...
                      params[FM_RANGE_CV_KNOB_PARAM].getValue(), 0.f, 100.f);
  ...
  void smoothControls(const EffectiveControls &targets) {
    ...
    smoothedControls.beatHz +=
        (targets.beatHz - smoothedControls.beatHz) * blend;
    smoothedControls.carrierHz +=
        (targets.carrierHz - smoothedControls.carrierHz) * blend;
    smoothedControls.amRateHz +=
        (targets.amRateHz - smoothedControls.amRateHz) * blend;
    smoothedControls.amDepth +=
        (targets.amDepth - smoothedControls.amDepth) * blend;
    smoothedControls.fmRateHz +=
        (targets.fmRateHz - smoothedControls.fmRateHz) * blend;
    smoothedControls.fmRangeHz +=
        (targets.fmRangeHz - smoothedControls.fmRangeHz) * blend;
  ...
  void process(const ProcessArgs &args) override {
    ...
    const float fmOffsetHz =
        std::sin(kTwoPi * fmPhase) * smoothedControls.fmRangeHz;
    const float lowEarHz =
        clampFrequency(smoothedControls.carrierHz + fmOffsetHz);
    const float highEarHz = clampFrequency(lowEarHz + smoothedControls.beatHz);
    const float amGain = 1.f - smoothedControls.amDepth * 0.5f *
                                   (std::sin(kTwoPi * amPhase) + 1.f);

    const float leftFrequencyHz =
        smoothedControls.leftHigh ? highEarHz : lowEarHz;
    const float rightFrequencyHz =
        smoothedControls.leftHigh ? lowEarHz : highEarHz;
    ...
    const float leftOutput =
        std::sin(kTwoPi * leftPhase) * amGain * kAudioLevelVolts;
    const float rightOutput =
        std::sin(kTwoPi * rightPhase) * amGain * kAudioLevelVolts;

    outputs[L_OUT_OUTPUT].setVoltage(leftOutput);
    outputs[R_OUT_OUTPUT].setVoltage(rightOutput);

    const float lightLevel = clamp(
        smoothedControls.beatHz / 100.f + smoothedControls.amDepth * 0.25f +
            smoothedControls.fmRangeHz / 100.f * 0.25f,
        0.f, 1.f);
...
struct BinauralBeaterWidget : ModuleWidget {
  BinauralBeaterWidget(BinauralBeater *module) {
    ...
    addParam(createParamCentered<RoundBlackKnob>(
        Vec(44.197, 348.902), module, BinauralBeater::LR_SWITCH_PARAM));
